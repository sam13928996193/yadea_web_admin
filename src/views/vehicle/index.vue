<template>
  ...
  <div class="app-container" >
    ...
    <!-- 选择车系 -->
     ...
     <!-- 选择车型 -->
      ...
      <el-form-item >
        <el-input v-model.trim="query.vinCode" placeholder="VIN码" ></el-input>
      </el-form-item>
  ...
  <edit :title="edit.title" :dialogVisible="edit.dialogVisible" 
    :pojo="edit.pojo" :vehicleSeriesOptions="vehicleSeriesOptions" :vehicleModelOptions="vehicleModelOptions" :vehicleConfigOptions="vehicleConfigOptions"  :remoteClose="remoteClose"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    openMenuSeries(item,index){
      ...
      fetchData(){
        vehicleApi.getList(this.query,this.page.current,this.page.size).then(response=>{
    ...
    remoteClose(){
     ...
     openAdd(){
      ...
      if (this.edit.currentVehicleConfigID.length ===0){
         this.$alert('请选择配置', '配置选择提示', {
          confirmButtonText: '确定'
        });
        return
      }
    ...
    handleEdit(id) {
      ...
      this.$confirm('确认删除这条记录吗, 是否继续?', '提示', {
          ...
          vehicleApi.deleteById(id).then(response=>{
             this.$message({
