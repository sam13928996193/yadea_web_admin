<template>
              ...
              <el-submenu :index="item.id"  @click.native="openMenu(item,index)">
                ...
                <div v-if="item.children.length!=0"  @click.stop>
                      <el-menu-item 
                        ...
                        >
                      <span slot="title">{{subItem.vehicleModelName}}</span>
                    </el-menu-item>
  ...
  <edit :title="edit.title" :dialogVisible="edit.dialogVisible" 
    :pojo="edit.pojo" :vehicleSeriesOptions="vehicleSeriesOptions" :vehicleModelOptions="vehicleModelOptions" :partCategoryOptions="partCategoryOptions"  :partFilterOptions="partFilterOptions" :remoteClose="remoteClose"/>
  </div>
</template>
...
import vehiclePartApi from '@/api/vehiclePart';
...
export default {
    name:'PartSoftWare',//和对应路由表中配置的name值一致
  ...
  methods: {
    // 刷新左边树形车系车型
      async fetchTreeData(){
          const {data} = await vehicleModelApi.getListTree();
          this.treeList=data
      },
      //刷新左边零件类别
    ...
    handleEdit(id) {
      partSoftwareApi.getById(id).then(response=>{
      ...
      this.$confirm('确认删除这条记录吗, 是否继续?', '提示', {
          ...
          partSoftwareApi.deleteById(id).then(response=>{
             this.$message({
