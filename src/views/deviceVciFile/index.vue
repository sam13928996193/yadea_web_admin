<template>
  <div class="app-container" >
    ...
    <devicevcifile-edit :title="edit.title" :dialogVisible="edit.dialogVisible" 
...
import DevicevcifileEdit from './deviceVciFileEdit';
...
export default ({
  ...
  components: {DevicevcifileEdit},

  methods: {
    fetchData(){
      deviceVciFileApi.getList(this.query,this.page.current,this.page.size).then(response=>{
    ...
    handleEdit(id) {
      deviceVciFileApi.getById(id).then(response=>{
    ...
    handleDelete(id) {
      this.$confirm('确认删除这条记录吗, 是否继续?', '提示', {
          ...
          deviceVciFileApi.deleteById(id).then(response=>{
             this.$message({
