<template>
    ...
    <el-form
      ...
      >
        ...
        <el-form-item label="配置编号" prop="vehicleConfigCode">
          <el-input v-model="pojo.vehicleConfigCode"></el-input>
        </el-form-item>
        <el-form-item label="配置名称" prop="vehicleConfigName">
          <el-input v-model="pojo.vehicleConfigName"></el-input>
        </el-form-item> 
  ...
  export default ({
    ...
    data() {
      return {
        rules: {
          ...
          vehicleConfigCode: [
            { required: true, message: '请输入配置代号', trigger: 'blur' },
            { min: 2, max: 50, message: '长度在 1 到 32 个字符', trigger: 'blur' }
          ],
          vehicleConfigName: [
    ...
    methods: {
      ...
      async submitData(){
        ...
        if(this.pojo.id){
          // 修改
          response = await vehicleConfigApi.update(this.pojo);
        }else{
          //新增
          response = await vehicleConfigApi.add(this.pojo);
        }
