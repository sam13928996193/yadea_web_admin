<template>
  ...
  <edit :title="edit.title" :dialogVisible="edit.dialogVisible" 
    :pojo="edit.pojo" :vehicleSeriesOptions="vehicleSeriesOptions" :vehicleModelOptions="vehicleModelOptions" :remoteClose="remoteClose"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    openMenu(item,index){
      ...
      fetchData(){
        vehicleConfigApi.getList(this.query,this.page.current,this.page.size).then(response=>{
    ...
    handleEdit(id) {
      vehicleConfigApi.getById(id).then(response=>{
      ...
      this.$confirm('确认删除这条记录吗, 是否继续?', '提示', {
          ...
          vehicleConfigApi.deleteById(id).then(response=>{
             this.$message({
