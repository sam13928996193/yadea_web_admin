...
<script>
  ...
  import partFileApi from '@/api/partSoftware';
  export default ({
    ...
    methods: {
      ...
      async submitData(){
        ...
        if(this.pojo.id){
          // 修改
          response = await partFileApi.update(this.pojo);
        }else{
          //新增
          response = await partFileApi.add(this.pojo);
        }
      ...
      uploadMainFile(file){
        ...
        partFileApi.uploadPartSoftwareFile(data).then(response=>{
      ...
      deletePartSoftwareFile(){
        ...
        if(this.pojo.fileUrl && this.pojo.fileUrl !== this.oldFileUrl){
           partFileApi.deletePartSoftwareFile(this.pojo.fileUrl);
