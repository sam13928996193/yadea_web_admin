<template>
  <div class="app-container" >
    ...
    <el-form :inline="true" :model="query" class="demo-form-inline" size="mini">
      <el-form-item >
        <el-input v-model.trim="query.partCategoryCode" placeholder="零件类型编号" ></el-input>
      </el-form-item>
      <el-form-item >
        <el-input v-model="query.partCategoryName" placeholder="零件类型描述"></el-input>
      </el-form-item>
...
export default ({
  name:'partCategory',//和对应路由表中配置的name值一致
  data () {
    return {
      list:[],
      page:{//分页对象
  ...
  methods: {
    fetchData(){
      partCategoryApi.getList(this.query,this.page.current,this.page.size).then(response=>{
        this.list = response.data.records;
        this.page.total = response.data.total;
      })
    },
    handleEdit(id) {
      partCategoryApi.getById(id).then(response=>{
        if(response.code===20000){
         
          this.edit.pojo = response.data;
          
          this.edit.title = '编辑系统模块'
    ...
    handleDelete(id) {
      this.$confirm('确认删除这条记录吗, 是否继续?', '提示', {
          ...
          partCategoryApi.deleteById(id).then(response=>{
             this.$message({
